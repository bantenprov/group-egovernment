...
export default {
  mounted() {
    axios.get('api/group-egovernment/' + this.$route.params.id)
      .then(response => {
        if (response.data.status == true) {
          ...
          this.model.created_at         = response.data.group_egovernment.created_at;
          this.model.updated_at         = response.data.group_egovernment.updated_at;
        } else {
          alert('Failed');
        }
      })
